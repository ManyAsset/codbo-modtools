	...
	if ( IsDefined( level.hostForcedEnd ) && level.hostForcedEnd )
	{
		for ( i = 0 ; i < playerRankings.size ; i++ )
		{
			if ( !playerRankings[i] IsLocalToHost() )
				playerRankings[i].wagerWinnings = level.wagerBet;
		}
	}
...
calculateTeamPayouts()
{
	...
	if( playersOnWinningTeam.size == 0 )
	{
		...
		setWagerWinningsOnPlayers( level.players, level.wagerbet );
		return;
	}
...
helpGameEnd()
{
	...
	for ( ;; )
	{
		...
		{
			if ( !players[i] isdemoclient() && IsDefined( players[i].pers["lives"] ) && ( players[i].pers["lives"] > 0 ) )
			{
				playersLeft++;
			}
		}
