...
init()
{
	...
	AssertEx( IsDefined(game["entity_headicon_allies"]), "Allied head icons are not defined.  Check the team set for the level.");
	AssertEx( IsDefined(game["entity_headicon_axis"]), "Axis head icons are not defined.  Check the team set for the level.");
...
setEntityHeadIcon(team, owner, offset, icon, constant_size) 
{
	...
	if ( IsDefined(owner) && !level.teamBased )
	{
		...
		if( !IsPlayer( owner ) )
		{
			AssertEx( IsDefined( owner.owner ), "entity has to have an owner if it's not a player");
			owner = owner.owner;
		}
